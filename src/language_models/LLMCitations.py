...
class LLMCitationPipeline(FewShotPipeline):
    def __init__(self, model, tokenizer, prompt: str, device = None, outputClass = Classification):
        super().__init__(model = model, tokenizer=tokenizer, device=device, prompt = prompt, outputClass=outputClass)